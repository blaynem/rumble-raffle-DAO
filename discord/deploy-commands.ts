...
const deployGlobalProdCommands = async () => {
  await rest.put(Routes.applicationCommands(APP_ID), { body: getAvailableSlashCommands(EnvFlags.PRODUCTION) })
}

const deployTestCommands = async (guildId: string) => {
  await rest.put(Routes.applicationGuildCommands(APP_ID, guildId), { body: getAvailableSlashCommands(EnvFlags.TEST) })
}
