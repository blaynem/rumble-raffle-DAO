...
const ToastMessage = ({ type, message, onClick }: ToastTypes) => {
  ...
  return (
    <div className="flex flex-col justify-center">
      <div className={`bg-${colors.main} shadow-lg mx-auto w-96 max-w-full text-base pointer-events-auto bg-clip-padding block mb-3`} id="toast" role="alert" aria-live="assertive" aria-atomic="true" data-mdb-autohide="false">
        <div className={`bg-${colors.main} flex justify-between items-center py-2 px-3 bg-clip-padding border-b border-${colors.secondary}`}>
          <p className="font-bold text-white flex items-center">{icon}&nbsp;{note}</p>
          <div className="flex items-center">
            <button onClick={onClick} type="button" className="btn-close btn-close-white box-content w-6 h-6 ml-2 text-white border-none rounded-none focus:shadow-none focus:outline-none focus:opacity-100 hover:text-white hover:opacity-75 hover:no-underline" data-mdb-dismiss="toast" aria-label="Close"><XIcon className="block h-6 w-6" aria-hidden="true" /></button>
          </div>
        </div>
        <div className={`p-3 bg-${colors.main} break-words text-white`}>
          {message}
        </div>
      </div>
    </div>
