...
const RumbleRoom = () => {
  ...
  const [activityLogRounds, setActivityLogRounds] = useState<EntireGameLog['rounds']>([])
  const [activityLogWinners, setActivityLogWinners] = useState<EntireGameLog['winners']>([])
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const { data }: { data: RoomDataType } = await fetch(
        `${BASE_WEB_URL}/api/rooms/${DEFAULT_GAME_ROOM}`
      ).then(res => res.json())
      setRoomData(data)
  ...
  const activityRounds = roomData?.params?.game_completed
    ? roomData?.gameData?.rounds!
    : activityLogRounds
  const activityWinners = roomData?.params?.game_completed
    ? roomData?.gameData?.winners!
    : activityLogWinners
    ...
    <div className={`${darkMode ? 'dark' : 'light'}`}>
      ...
      >
        <div>
          ...
          {isRoomCreator && <AdminRoomPanel roomSlug={roomSlug} />}
          {roomData?.params?.game_completed && (
        ...
        <div className="flex flex-col md:flex-row sm:flex-row">
          ...
          >
            ...
            <DisplayKillCount entrants={entrants} rounds={activityRounds} userId={user.id} />
          </div>
          {/* Right Side */}
          <div
          ...
          >
            ...
            <div className="flex flex-col items-center pb-12">
              <DisplayActivityLogs allActivities={activityRounds} userId={user.id} />
              {activityWinners.length > 0 && (
                <DisplayWinners winners={activityWinners} userId={user.id} />
              )}
            </div>
          </div>
        </div>
