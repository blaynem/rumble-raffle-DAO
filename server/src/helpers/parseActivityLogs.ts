 ...
 */
const mapRumbleKillCountToDecimals = (
  killCount: ActivityLogType['killCount']
): SingleActivity['kill_count'] => {
...
export const parseActivityLogForClient = (
  gameActivityLogs: GameEndType['gameActivityLogs'],
  gamePlayers: PickFromPlayers[]
): EntireGameLog => {
  ...
  const rounds: EntireGameLog['rounds'] = gameActivityLogs
    .map(round => {
      ...
      const activities: SingleActivity[] = round.activityLog.map(
        ({ activity, activityId, participants, killCount }, index) => ({
          ...
          id: activityId,
          participants: participants.map(player => gamePlayers.find(p => p.id === player)) as (
            | PickFromPlayers
            | DiscordPlayer
          )[],
    ...
    .filter(Boolean) as RoundActivityLog[]
  return {
    rounds,
    winners
  }
