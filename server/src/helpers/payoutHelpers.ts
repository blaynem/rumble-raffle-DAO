...
export const payoutTemplate = ({
  ...
  payment_reason,
  notes
}: PayoutTemplateType): PayoutsOmitId => ({
  public_address,
  payment_amount,
  payment_reason,
  notes,
...
export const payoutNotesTemplate = (
  {
    ...
    gameKills,
    gamePayouts
  }: { id: string; gameKills: GameEndType['gameKills']; gamePayouts: PrizePayouts },
  extraNotes?: string
) =>
...
export const selectPayoutFromGameData = (
  ...
  return payouts
    .filter(payout => payout.payment_amount.toNumber() > 0)
 ...
 * @returns
 */
const getKillPayout = (id: string, gamePayouts: PrizePayouts): Prisma.Decimal =>
  gamePayouts.kills[id] || new Prisma.Decimal(0)
 ...
 * @returns kill notes
 */
const getKillNotes = (killCount: number, killPayout: Prisma.Decimal) =>
  killCount > 0 ? `Total kill payout: ${killPayout}. Total kill count: ${killCount}.` : ''
