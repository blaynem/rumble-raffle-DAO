 ...
 */
const findPlayerById = (pubAddress: string, players: (PickFromPlayers | DiscordPlayer)[]) =>
  players.find(player => player.id === pubAddress)
 ...
 * @returns
 */
const getKillCountsFromPlayers = (
  killCounts: Prisma.Decimal[],
  players: string[]
): { [playerId: string]: Prisma.Decimal } => {
  //  const killCountConverted = killCounts.map(count => count.toNumber());
  return killCounts?.reduce(
    (acc: { [playerId: string]: Prisma.Decimal }, curr: Prisma.Decimal, index) => {
 ...
 */
const getRoundsData = (
  allGameLogs: RoundsType[],
  players: (PickFromPlayers | DiscordPlayer)[]
): EntireGameLog['rounds'] => {
  ...
  sorted.forEach(round => {
    ...
    const singleActivity: SingleActivity = {
      ...
      participants: round.participants.map((id: string) => findPlayerById(id, players)) as (
        | PickFromPlayers
        | DiscordPlayer
      )[]
    }
...
export const selectRoomInfo = (roomInfo: RoomDataType): RoomDataType => {
  ...
  const winners: EntireGameLog['winners'] = roomInfo.params.winners.map((id: string) =>
    findPlayerById(id, allPlayers)
  ) as (PickFromPlayers | DiscordPlayer)[]
