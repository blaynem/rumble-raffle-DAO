...
export const startRumble = async (roomSlug: string): Promise<EntireGameLog | { error: any }> => {
  try {
    ...
    const { discordPlayers, roomData: localRoomData } = room
    ...
    if (findDiscordPlayersById.length > 0) {
      ...
      await prisma.$transaction(
        findDiscordPlayersById.map(player =>
    ...
    const notFoundDiscordPlayers = discordPlayers.filter(
      player => !foundPlayers.includes(player.id)
    )

    // Get game data from db, including all players
    const { data: roomData } = await getGameDataFromDb(roomSlug)
    if (!roomData || roomData.params.game_completed || roomData.params.game_started) {
      throw Error('There was no room data, or the game has already started and been completed.')
    }
    // Get all activities for the game.
    const activityFetch = await getAllActivities()
    if (!activityFetch.data) {
      throw Error(activityFetch.error)
    }

    // Combine players from db + leftover discord players
    const combinedPlayers = [...roomData.players, ...notFoundDiscordPlayers]
