...
router.post(
  PATH_VERIFY_INIT,
  jsonParser,
  async (req: AuthDiscordInitPostBody, res: express.Response<AuthDiscordInitPostResponse>) => {
...
router.get(
  '/:verification_id',
  async (req: AuthDiscordVerifyGetBody, res: express.Response<AuthDiscordVerifyGetResponse>) => {
...
router.post(
  PATH_VERIFY,
  jsonParser,
  async (req: AuthDiscordVerifyPostBody, res: express.Response<AuthDiscordVerifyPostResponse>) => {
    try {
      ...
      const authEntry = authStore.get(verification_id)
      if (!authEntry) {
        res.status(400).json({ data: null, error: 'Unable to find verification code.' })
        return
      }

      const { discord_id, discord_tag } = authEntry
...
router.post(
  ...
  jsonParser,
  async (
    req: express.Request<{ body: { discord_id: string } }>,
    res: express.Response<AuthDiscordVerifyPostResponse>
  ) => {
    try {
      const { discord_id } = req.body
      // If any of the fields are missing, throw an error
