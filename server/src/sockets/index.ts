...
function joinRoom(roomSlug: string) {
  ...
  if (roomData.game_started) {
    ...
    io.to(this.id).emit(UPDATE_ACTIVITY_LOG_ROUND, roomData.gameData.rounds);
    io.to(this.id).emit(UPDATE_ACTIVITY_LOG_WINNER, roomData.gameData.winners);
  }
}
...
async function startGame(data: { playerData: definitions["users"]; roomSlug: string }) {
  const {roomData} = availableRoomsData[data.roomSlug];
  ...
  if (!roomData || roomData.game_started || roomData.players.length < 1) {
    console.log('---startRumble--ERROR', data.roomSlug);
    return;
  }
  const gameData = await startRumble(data.roomSlug);
  roomData.gameData = gameData;
  io.in(data.roomSlug).emit(UPDATE_ACTIVITY_LOG_ROUND, gameData.rounds)
  io.in(data.roomSlug).emit(UPDATE_ACTIVITY_LOG_WINNER, gameData.winners)
...
async function clearGame(data: { playerData: definitions["users"]; roomSlug: string }) {
  ...
  const roomsRes = await client.from<definitions["rooms"]>('rooms').update({ game_started: false, winners: null }).match({ id: roomData.id });
